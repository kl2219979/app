export const AccountsView = {
            ...
            <div class="space-y-6 animate-fade-in">
                <div class="glass-card p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <button onclick="window.openAccountModal()" class="flex items-center gap-2 px-4 py-2 border border-indigo-500/30 hover:border-indigo-500 bg-indigo-500/10 text-indigo-400 rounded-xl text-sm font-semibold">
                            <i data-lucide="plus" class="w-4 h-4"></i> Nueva cuenta
                        </button>
                    </div>
                </div>
                <div id="accounts-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"></div>
            </div>
        `;
    },
    ...
    renderGrid(state, utils) {
        ...
        list.forEach((a) => {
            ...
            card.className = `glass-card p-6 rounded-2xl border ${active ? 'border-slate-800/80' : 'border-slate-700/40 opacity-70'}`;
            card.innerHTML = `
                ...
                <p class="text-2xl font-extrabold text-white mb-4">${utils.formatCurrency(a.saldo, a.moneda)}</p>
                <div class="flex justify-end gap-2 pt-3 border-t border-slate-800/50">
                    ${active ? `
                    ...
                    ` : `
                        <button onclick="window.reactivateAccountFlow(${a.id})" class="text-xs font-bold text-emerald-400 hover:text-emerald-300 px-2 py-1">Reactivar</button>
                    `}
                </div>
            `;
